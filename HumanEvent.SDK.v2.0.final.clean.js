 ...
 */\1\2 {
  ...
  /* region Physio / Cognition */\1
/**
 * circadianRhythm function
 * @param t
 * @returns {any}
 */\1\2 {
...
/*!
 * HumanEvent SDK v1.4.0
 * 行为与动作库 · 拟人化自动化 · 工业级结构 · 学术可读性
 * ------------------------------------------------------------------
 * 支持行为:click, move, hover, scroll, drag, type, focus, blur
 * 特性:前摇/后摇、拟人延迟建模、贝塞尔曲线、链式调用、热插拔、可扩展接口
 * ------------------------------------------------------------------
 * 所有行为均可组合调用,模块化设计,适用于浏览器与嵌入式系统
 */
 ...
 */\1\2 {
    ...
    e.initMouseEvent(type, true, true, win, 0,
      cfg.clientX, cfg.clientY, cfg.clientX, cfg.clientY,
      false, false, false, false, cfg.button || 0, null);
    el.dispatchEvent(e);
  }

  // 延迟模型(简化版)
  HumanEvent.delay = function(opts) {
    var base = 500;
    var jitter = Math.random() * 300;
    return { delay: Math.round(base + jitter) };
  };

  // 通用行为执行器\1
/**
 * runBehavior function
 * @param pre, act, post
 * @returns {any}
 */\1\2 {
    var seq = [];
    if (pre) seq.push(function(next) { sleep(pre).then(next); });
    seq.push(function(next) { act(); next(); });
    if (post) seq.push(function(next) { sleep(post).then(next); });

    (function run(i) {
      if (i < seq.length) seq[i](function() { run(i + 1); });
    })(0);
  }

  // click 行为
  HumanEvent.click = function(el, opts) {
    opts = opts || {};
    var pos = center(el);
    runBehavior(opts.preDelay, function() {
      fire('mousedown', el, pos);
      fire('mouseup', el, pos);
      fire('click', el, pos);
    }, opts.postDelay);
    return HumanEvent;
  ...
  // move 行为
  HumanEvent.move = function(from, to, opts) {
    opts = opts || {};
    var steps = opts.steps || 10;
    var cp1x = from.x + (to.x - from.x) * 0.3;
    var cp1y = from.y + (to.y - from.y) * 0.3;
    var cp2x = from.x + (to.x - from.x) * 0.7;
    var cp2y = from.y + (to.y - from.y) * 0.7;
    ...
    runBehavior(opts.preDelay, function() {
      for (var i = 0; i <= steps; i++) {
        (function(i) {
          setTimeout(function() {
            var t = i / steps;
            var x = bezier(t, from.x, cp1x, cp2x, to.x);
            var y = bezier(t, from.y, cp1y, cp2y, to.y);
            fire('mousemove', doc.body, { clientX: x, clientY: y });
          }, i * 20);
        })(i);
      }
    }, opts.postDelay);
  ...
  // hover 行为
  HumanEvent.hover = function(el, opts) {
    opts = opts || {};
    var pos = center(el);
    runBehavior(opts.preDelay, function() {
      fire('mouseenter', el, pos);
      fire('mouseover', el, pos);
    }, opts.postDelay);
    return HumanEvent;
  };

  // scroll 行为
  HumanEvent.scroll = function(el, opts) {
    opts = opts || {};
    runBehavior(opts.preDelay, function() {
      el.scrollTop += opts.dy || 100;
      el.scrollLeft += opts.dx || 0;
    }, opts.postDelay);
    return HumanEvent;
  };

  // drag 行为
  HumanEvent.drag = function(el, target, opts) {
    opts = opts || {};
    var from = center(el);
    var to = center(target);
    runBehavior(opts.preDelay, function() {
      fire('mousedown', el, from);
      HumanEvent.move(from, to, { steps: opts.steps || 10 });
      setTimeout(function() {
        fire('mouseup', target, to);
      }, (opts.steps || 10) * 20 + 50);
    }, opts.postDelay);
    return HumanEvent;
  };
  ...
  // type 行为
  HumanEvent.type = function(el, opts) {
    opts = opts || {};
    var text = opts.text || '';
      ...
      for (var i = 0; i < text.length; i++) {
        (function(i) {
          setTimeout(function() {
            ...
            e.initKeyboardEvent('keypress', true, true, win, false, false, false, false, text.charCodeAt(i), 0);
            el.dispatchEvent(e);
            el.value += text[i];
          }, i * interval);
        })(i);
  ...
  // focus 行为
  HumanEvent.focus = function(el, opts) {
    opts = opts || {};
    runBehavior(opts.preDelay, function() {
      el.focus();
    }, opts.postDelay);
  ...
  // blur 行为
  HumanEvent.blur = function(el, opts) {
    opts = opts || {};
    runBehavior(opts.preDelay, function() {
      el.blur();
    }, opts.postDelay);
  ...
  // 可扩展接口挂载点
  HumanEvent.extend = function(name, fn) {
